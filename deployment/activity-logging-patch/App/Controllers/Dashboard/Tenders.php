...
class Tenders extends \Core\Controller
{
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            logActivity('Created', 'tender', $data['title'] ?? '');
    ...
    public function updateAction()
    {
        ...
        try {
            ...
            logActivity('Updated', 'tender', $data['title'] ?? $data['id'] ?? '');
    ...
    public function deleteAction()
    {
        ...
        try {
            ...
            logActivity('Deleted', 'tender', "ID: {$id}");
    ...
    public function switchStatus():void
    {
        ...
        try {
            $id =  TenderModel::Update($data);
            logActivity('Switched status', 'tender', $data['id'] ?? '');
            $_SESSION['success'] = ['message' => 'successfully updated record!'];
        } catch (\Throwable $th) {
            echo $th->getMessage();
        }
    ...
    public function archiveExpiredAction()
    {
        try {
            ...
            if ($count > 0) {
                logActivity('Archived expired', 'tenders', "{$count} archived");
